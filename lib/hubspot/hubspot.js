...
hubspotAPI.prototype.isTokenValid = function(access_token, callback) {
  ...
  this.get(path, availableParams, params, function (err, data) {
...
hubspotAPI.prototype.createOrUpdateTimelineEvent = function(eventTypeId, email, eventId, extraData, callback) {
  ...
  self.put(path, availableParams, params, function (err, result) {
    if (err) {
      ...
      if(err.statusCode === 401) {
        self.resetAccessToken(self.refresh_token, function (err, tokenInfo) {
          ...
          self.put(path, availableParams, params, callback);
        });
      } else {
        return callback(err, result);
      }
